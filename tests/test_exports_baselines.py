...
class ExportAndBaselineTest(unittest.TestCase):
    ...
    def test_published_ledger_freezes_prediction_values_and_builds_static_site(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((result.output_dir / "llms.txt").exists())
            ...
            self.assertIn('<div class="bar bar--series" role="img" aria-label="6/10 aus gespielten Spielen">', html)
            ...
            self.assertIn('<span class="bar__legend-item numeric">6/10 aus gespielten Spielen</span>', html)
            ...
            self.assertIn("# Helga World Cup Predictions", llms_txt)
            self.assertIn("[Predictions JSON API](http://127.0.0.1:8000/api/predictions)", llms_txt)
            self.assertIn("[English overview](http://127.0.0.1:8000/en/)", llms_txt)
            self.assertIn("[Tournament forecast](http://127.0.0.1:8000/en/tournament-forecast)", llms_txt)
            self.assertIn("Prefer fixture keys, FIFA team codes, and JSON fields", llms_txt)
            ...
            self.assertIn("Der SRF-Tipp entspricht dem wahrscheinlichsten Resultat 1:0 (12.3%).", detail)
            ...
            self.assertIn('<p class="detail-hero__score" data-result="trend">', detail)
            self.assertIn('<span class="detail-hero__score-value numeric">2:0</span>', detail)
            self.assertIn('<span class="detail-hero__score-label">Schlussresultat</span>', detail)
            ...
            self.assertIn('<span class="heatmap__cell-marker" data-marker="actual" aria-hidden="true"></span>', detail)
            self.assertIn('<span class="heatmap__dot" data-marker="actual" aria-hidden="true"></span><span>Grün markiert: Schlussresultat 2:0 (0.0%).</span>', detail)
            ...
            self.assertIn("Grün markiert: Schlussresultat 2:0 (0.0%).", detail)
            ...
            self.assertIn(".summary-tooltip--inline {\n  top: auto;", css)
            self.assertIn("bottom: var(--tooltip-inline-bottom, calc(100% + var(--space-md)));", css)
            self.assertIn(".summary-tooltip--inline .summary-tooltip__arrow {\n  top: 100%;\n  bottom: auto;", css)
            self.assertIn(".summary-tooltip__arrow", css)
            self.assertIn("--tooltip-arrow-anchor: calc(65 / 340 * 100%);", css)
            ...
            self.assertIn("border: 4px solid transparent;", css)
            self.assertIn(".heatmap__cell[data-most-likely=\"true\"] {\n  border-color: var(--focus-ring);", css)
            self.assertIn(".heatmap__cell-marker {\n  position: absolute;", css)
            self.assertIn("box-shadow: 0 0 0 2px var(--paper);", css)
            self.assertIn(".heatmap__dot {\n  display: inline-block;\n  flex: 0 0 auto;\n  width: 7px;\n  height: 7px;\n  border-radius: 50%;", css)
            self.assertIn(".heatmap__dot[data-marker=\"most-likely\"] {\n  background: var(--focus-ring);", css)
            self.assertIn(".heatmap__dot[data-marker=\"actual\"] {\n  background: var(--accent);", css)
