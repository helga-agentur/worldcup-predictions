...
class ExportAndBaselineTest(unittest.TestCase):
    ...
    def test_published_ledger_freezes_prediction_values_and_builds_static_site(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('<dd class="summary__segments" aria-label="0 exakt · 1 richtig · 0 falsch">', html)
            self.assertIn('<span class="summary__segment" data-result="trend">', html)
            self.assertIn('<span class="summary__segment-value">1</span>', html)
            self.assertIn('<span class="summary__segment-label">Richtig</span>', html)
            ...
            self.assertIn('<dd class="summary__segments" aria-label="0 exact scores · 1 correct outcomes · 0 wrong outcomes">', en_html)
            self.assertIn('<span class="summary__segment-label">Correct</span>', en_html)
            ...
            self.assertIn(".summary__segments", css)
            self.assertIn(".summary__segment", css)
