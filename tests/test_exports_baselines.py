...
class ExportAndBaselineTest(unittest.TestCase):
    ...
    def test_published_ledger_freezes_prediction_values_and_builds_static_site(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('<meta property="og:site_name" content="Helga Tippspiel Prognosen">', html)
            ...
            self.assertIn("Helga World Cup Pool Predictions", en_html)
            ...
            self.assertIn('<a href="/en/matches/future" lang="en"', de_future_html)
            self.assertIn('<title>Kommende Spiele — Helga Tippspiel Prognosen</title>', de_future_html)
            ...
            self.assertIn(
                '<p class="lead">Provider-neutrale Score-Prognosen, SRF-Optimierung und 20min-Optimierung aus dem aktuellen Datenlauf.</p>',
                html,
            )
            ...
            self.assertIn('<dt class="summary-label">Trefferquote</dt>', html)
            self.assertIn('<dd class="summary-value">100%</dd>', html)
            self.assertIn("0 exakt · 1 Tendenz · 0 daneben", html)
            self.assertIn('<dd class="summary-hitbar" aria-hidden="true">', html)
            self.assertIn('<dt class="summary-label">Offene Tipps</dt>', html)
            self.assertIn("Ø 6.0 pro gewertetem Spiel", html)
            self.assertIn("Hit rate", en_html)
            self.assertIn("0 exact · 1 trend · 0 missed", en_html)
            ...
            self.assertIn('<nav class="resource-links" aria-label="Ressourcen">', html)
            self.assertIn('<a href="/de/turnier">Turnierprognose</a>', html)
            self.assertIn('<a href="/api/predictions" target="_blank" rel="noopener" data-analytics-event="helga_api_click">JSON API</a>', html)
            future_section = html.split('<section class="table-section" aria-labelledby="future-title">', 1)[1].split(
                '<section class="table-section" aria-labelledby="past-title">',
                1,
            )[0]
            past_section = html.split('<section class="table-section" aria-labelledby="past-title">', 1)[1]
            self.assertIn('<p class="section-actions">', future_section)
            self.assertIn('<a class="content-link" href="/de/spiele/kommende">Alle kommenden Spiele</a>', future_section)
            self.assertIn('<p class="section-actions">', past_section)
            ...
            self.assertIn('<span class="hit-chip hit-trend">Tendenz</span>', past_section)
            ...
            self.assertIn('<title>Brasilien - Japan — Helga Tippspiel Prognosen</title>', detail)
            self.assertIn('"@type": "SportsEvent"', detail)
            self.assertIn('"startDate": "2026-07-10T18:00:00Z"', detail)
            self.assertIn('<h1 id="match-title" class="detail-title">', detail)
            h1_match_name = detail.split('<h1 id="match-title" class="detail-title">', 1)[1].split("</h1>", 1)[0]
            self.assertIn("Brasilien – Japan", h1_match_name)
            self.assertIn('<span class="detail-flag" aria-hidden="true">🇧🇷</span>', h1_match_name)
            self.assertIn("Brazil – Japan", en_detail.split('<h1 id="match-title" class="detail-title">', 1)[1].split("</h1>", 1)[0])
            self.assertIn('<div class="prob-bar" role="img"', detail)
            ...
            self.assertIn('<span class="verdict-score numeric">2:0</span>', detail)
            self.assertIn('<span class="hit-chip hit-trend">Tendenz</span>', detail)
            self.assertIn("+6 P.", detail)
            self.assertIn("Resultat-Matrix", detail)
            self.assertIn('<table class="heatmap-table">', detail)
            self.assertIn('class="heatmap-cell heatmap-cell-hot heatmap-cell-ml"', detail)
            self.assertIn('title="1:0 — 12.3%"', detail)
            self.assertIn('heatmap-cell-actual', detail)
    ...
    def test_site_counts_unpredicted_fixtures_and_keeps_locked_rows_out_of_upcoming_section(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('<span class="card-team-name">Sieger Sechzehntelfinal 4</span>', future_section)
    ...
    def test_site_ignores_stale_fixture_placeholders_from_old_source_batches(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('<span class="card-team-name">Kanada</span>', html)
            self.assertIn('<span class="card-team-name">Sieger Sechzehntelfinal 4</span>', html)
    ...
    def test_site_ignores_unpredicted_placeholder_when_prediction_covers_same_slot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.future_count, 2)
            self.assertIn('<span class="card-team-name">Marokko</span>', html)
            self.assertIn('<span class="card-team-name">Sieger Sechzehntelfinal 4</span>', html)
            self.assertIn('<span class="card-team-name">Morocco</span>', en_html)
            self.assertIn('<span class="card-team-name">Winner Round of 32 match 4</span>', en_html)
            self.assertNotIn("W75", html)
