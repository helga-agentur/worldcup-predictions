...
SLOT_CODE_RE = re.compile(r"^(?:W|RU)\d{1,3}$", re.I)
...
def canonical_slot_code(value: object) -> str:
    ...
    if loser:
        return _canonical_loser_slot(int(loser.group("number")))
    ...
    if round_slot:
        ...
        if round_name:
            ...
            if match_number is not None:
                if outcome in {"winner", "sieger"}:
                    return f"W{match_number}"
                return _canonical_loser_slot(match_number)
    return ""
...
def slot_display_name(value: object, *, locale: str = "de") -> str:
    ...
    if not code:
        return ""
    prefix, number = _split_slot_code(code)
...
def _split_slot_code(code: str) -> tuple[str, int | None]:
    if code.startswith("RU"):
        return "RU", int(code[2:])
    if code.startswith("W"):
        return "W", int(code[1:])
    return "", None


def _canonical_loser_slot(match_number: int) -> str:
    """Return FIFA's loser placeholder where FIFA defines one."""

    return f"RU{match_number}" if match_number in {101, 102} else ""
