...
### Failure Backoff and Run Circuit

Failed requests back off on an escalating ladder of 1, 4, 12, and 24 hours, counted per request key from consecutive failures since the last success. The 24-hour cap doubles as a daily probe, so a recovered source is picked up within a day no matter how long it was down. Three exceptions to the ladder:

- A `Retry-After` header always wins; the server knows its own reset time.
