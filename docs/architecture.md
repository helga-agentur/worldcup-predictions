...
Tournament simulation is provider-neutral. It consumes fixtures, already-known scores, team strengths, and exact-score matrices from the prediction model. Known scores are fixed; only unresolved fixtures are sampled. This lets the same simulator run before the first match or midway through the tournament without changing provider code.
