...
Raw result observations stay in `tournament_results`. A result enters `TournamentState.results` only when the exact score is confirmed by at least three independent source families, or by at least two high-authority sources (`srf_public`, `fifa_match_centre`, or `football_data_org`). Sources that share one extractor count as a single family — every `dynamic_public:<domain>` row is one correlated witness, not many. When competing scores are both confirmable, the score backed by more high-authority sources wins before raw source counts are compared. Result observations whose kickoff differs from the canonical fixture by up to `CONFIRMED_RESULT_KICKOFF_WINDOW_HOURS` are pooled onto that fixture, so kickoff-time disagreements cannot confirm a phantom duplicate match. Website publishing, standings, live calibration, provider points, and model learning consume that confirmed state; lower-consensus rows remain internal diagnostics in `tournament_result_checks`.
...
- `public_score_sources` checks allowed public result and page-analysis pages from FIFA, FotMob, SofaScore, and 20min through robots.txt-aware requests. Pages contribute diagnostics unless their allowed paths expose canonical final-score or supported analysis/stat text. Private or robots-disallowed APIs are not used. (ESPN's scoreboard was removed 2026-07-10 after an audit showed zero successful fetches all tournament.)
- `dynamic_public_sources` works from an audited trusted source registry, follows a bounded set of same-domain tournament links, and stores page metadata/fingerprints plus extracted fixture, result, and market claims. The trusted registry rotates in per-run batches so broad discovery stays bounded per cron run. It uses Scrapy selectors when available for HTML parsing, while all HTTP requests still go through `SourceRuntime` for source-ledger freshness, cache validators, and backoff. Dynamic result claims are not high-authority; they become `tournament_results` observations only after multi-domain weighted support, then still need the central result-consensus policy before tournament state changes.
